...
namespace MSyics.Cacheyi
{
    ...
    public class CacheStore<TUnique, TKey, TValue>
    {
        ...
        private CacheProxy<TUnique, TKey, TValue> Alloc(CacheKey<TUnique, TKey> key)
        {
            ...
            using (m_cacheLock.Scope(LockStatus.Write))
            {
                ...
                var item = new CacheProxy<TUnique, TKey, TValue>()
                {
        ...
        /// <param name="key">キャッシュを区別するためのキー</param>
        /// <returns>キャッシュを関連付けるオブジェクト</returns>
        public CacheProxy<TUnique, TKey, TValue> Alloc(TKey key)
        {
            #region Doer
            if (key == null) { throw new ArgumentNullException(nameof(key)); }
            #endregion
        ...
        /// <param name="keys">キャッシュを区別するためのキーの一覧</param>
        /// <returns>キャッシュを関連付けるオブジェクトの一覧</returns>
        public IEnumerable<CacheProxy<TUnique, TKey, TValue>> Alloc(IEnumerable<TKey> keys)
        {
            if (keys == null)
            {
                yield break;
        ...
        /// <param name="cache">キャッシュを関連付けるオブジェクト。取得に失敗した場合は null を設定します。</param>
        /// <returns>取得に成功したかどうかを示す値</returns>
        public bool TryAlloc(TKey key, out CacheProxy<TUnique, TKey, TValue> cache)
        {
            if (key == null)
            {
                cache = null;
